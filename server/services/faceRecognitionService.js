...
export const verifyLiveness = (faceMetrics) => {
  const { spoofIndex, challengeType, blinkDetected, headTurnRatio, landmarks } = faceMetrics;
  ...
  if (challengeType) {
    if (challengeType === 'blink') {
      if (!blinkDetected) {
        return {
          passed: false,
          reason: 'Liveness challenge failed: Blink not detected.'
        };
      }
    } else if (challengeType === 'turn_left' || challengeType === 'turn_right') {
      // Calculate server-side ratio if landmarks are provided
      let ratio = headTurnRatio;
      if (landmarks && Array.isArray(landmarks) && landmarks.length === 3) {
        const jawLeft = landmarks[0];
        const jawRight = landmarks[1];
        const noseTip = landmarks[2];
        if (jawLeft && jawRight && noseTip && (jawRight.x - jawLeft.x) !== 0) {
          ratio = (noseTip.x - jawLeft.x) / (jawRight.x - jawLeft.x);
        }
      }

      if (challengeType === 'turn_left') {
        if (ratio === undefined || ratio > 0.38) {
          return {
            passed: false,
            reason: 'Liveness challenge failed: Head turn LEFT not detected or insufficient.'
          };
        }
      } else if (challengeType === 'turn_right') {
        if (ratio === undefined || ratio < 0.62) {
          return {
            passed: false,
            reason: 'Liveness challenge failed: Head turn RIGHT not detected or insufficient.'
          };
        }
      }
    }
...
export const identifyFace = async (incomingDescriptor, threshold = 0.70) => {
  ...
  if (cachedBiometrics) {
    ...
    if (isSupabaseLive) {
      const { data, error } = await supabase.from('employees').select('id, name, department, avatar, face_data').not('face_data', 'is', null);
    ...
    if (employees.length === 0) {
      employees = await db.all(`SELECT id, name, department, avatar, face_data FROM employees WHERE face_data IS NOT NULL`);
    }

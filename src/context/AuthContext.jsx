...
export const AuthProvider = ({ children }) => {
  ...
  const handleLogin = async (emailOrId, password) => {
    try {
      ...
      // Hardcoded Admin & Employee Bypass for Instant Reliable Authentication
      ...
      if ((normalizedEmail === 'employee@company.com' || normalizedEmail === 'oes/038') && password === 'employeepassword') {
        console.log('[AUTH BYPASS]: Logging in with Demo Employee credentials.');
        let isFaceReg = false;
        try {
          const { data: empDb } = await supabase.from('employees').select('face_data').eq('email', 'employee@company.com').maybeSingle();
          isFaceReg = empDb?.face_data !== null && empDb?.face_data !== undefined;
        } catch (e) {}
        ...
        const mockEmpUser = {
          ...
          is_face_registered: isFaceReg
        };
        
        localStorage.setItem('quantum_token', 'mock-employee-token-9824');
        localStorage.setItem('quantum_user', JSON.stringify(mockEmpUser));
        
        if (mountedRef.current) {
          setToken('mock-employee-token-9824');
          setUser(mockEmpUser);
          setIsAuthenticated(true);
        }
  ...
  const handleUpdateUser = (updatedFields) => {
    setUser(prev => {
      ...
      try {
        localStorage.setItem('quantum_user', JSON.stringify(nextUser));
      } catch (e) {}
      return nextUser;
    });
  };

  const handleLoginCallback = React.useCallback(handleLogin, []);
  const handleRegisterCallback = React.useCallback(handleRegister, [token]);
  const handleLogoutCallback = React.useCallback(handleLogout, []);
  const handleUpdateUserCallback = React.useCallback(handleUpdateUser, []);
  ...
  const authValue = React.useMemo(() => ({
    ...
    isAuthenticated,
    loading,
    login: handleLoginCallback,
    register: handleRegisterCallback,
    logout: handleLogoutCallback,
    updateUser: handleUpdateUserCallback
  }), [user, token, isAuthenticated, loading, handleLoginCallback, handleRegisterCallback, handleLogoutCallback, handleUpdateUserCallback]);
